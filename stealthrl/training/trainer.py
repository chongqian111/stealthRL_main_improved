...
class StealthRLTrainer:
    ...
    def __init__(
        ...
        # Trainer will be initialized in train() when dataset is available
        self.trainer = None
        
    def _initialize_trainer(self, dataset=None):
        ...
        if self.algorithm == "grpo":
            ...
            # Wrap composite reward for new TRL API: (prompts, completions) -> List[float]
            ...
            def reward_func(prompts, completions, **kwargs):
                ...
                for completion in completions:
                    try:
                        ...
                        rewards.append(float(r.item() if hasattr(r, "item") else r))
                    except Exception:
                        rewards.append(0.0)
                return rewards

            # Ensure dataset has non-empty "prompt" column required by GRPOTrainer
            if dataset is not None:
                def fix_prompt(x):
                    p = x.get("prompt", "")
                    # Use text as prompt if prompt is empty
                    if not p or not p.strip():
                        p = x.get("text", "")
                    return {"prompt": p}
                dataset = dataset.map(fix_prompt)
                # Filter out any remaining empty prompts
    ...
    def train(self, dataset: Dataset, eval_dataset: Optional[Dataset] = None):
        ...
        # Initialize trainer now that we have the dataset
        self._initialize_trainer(dataset)
        ...
        if self.algorithm == "grpo":
            ...
            # Train using GRPO (dataset and reward_funcs already passed at init)
            self.trainer.train()
